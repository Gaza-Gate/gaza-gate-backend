...
const registerConversationHandlers = (io, socket) => {
  ...
  socket.on("send_message", async ({ conversationId, content, productId } = {}) => {
    try {
      if (!conversationId) {
        socket.emit("error", { message: "conversationId is required." });
        return;
      ...
      const message = await conversationService.sendMessage(socket.userId, conversationId, {
        content,
        productId,
      });
  ...
  socket.on("typing", async ({ conversationId, isTyping } = {}) => {
    try {
      ...
      const conversation = await conversationService.loadConversationOrFail(
        conversationId,
      );
      conversationService.assertParticipant(conversation, socket.userId);
      ...
      socket
        ...
        .emit("typing", { conversationId, userId: socket.userId, isTyping: !!isTyping });
    } catch {
      // Typing is best-effort; ignore failures.
    }
  });

  socket.on("update_message", async ({ conversationId, messageId, content } = {}) => {
    try {
