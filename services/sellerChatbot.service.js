...
const { CHATBOT_RECORD_TYPES } = require("../constants/chatbot.constant.js");
const {
...
const chat = async (userId, message, sessionId, file) => {
  ...
  const agentResult = await sellerAiAgent.runAgent(userId, history, displayMessage, {
    sessionId: session.id,
...
const getSessions = async (userId, query = {}) => {
  ...
  const { count, rows } = await ChatbotRecord.findAndCountAll({
    ...
    attributes: ["id", "content", ["created_at", "createdAt"], ["updated_at", "updatedAt"]],
...
const getSessionMessages = async (userId, sessionId) => {
  ...
  const messages = await ChatbotRecord.findAll({
    ...
    attributes: ["id", "role", "content", "toolName", ["created_at", "createdAt"]],
    order: [["created_at", "ASC"]],
  });
