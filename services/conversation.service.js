...
const getNotificationService = () => require("./notification.service.js");
...
const syncConversationLastMessage = async (conversationId, transaction = null) => {
  const findOptions = {
    where: { conversationId },
...
const updateMessage = async (userId, conversationId, messageId, { content }) => {
  const conversation = await loadConversationOrFail(conversationId);
  assertParticipant(conversation, userId);
  ...
  if (normalizeUserId(conversation.lastMessageId) === normalizeUserId(messageId)) {
...
const markAsRead = async (userId, conversationId) => {
  ...
  await Conversation.update({ [column]: lastReadAt }, { where: { id: conversationId } });
