...
const dashboardService = require("../../overview/seller/dashboard.service.js");
...
const resolveCategoryId = async (categoryId, categoryName) => {
  ...
  if (!categoryName?.trim()) {
    throw AppError.fail(
      "Category is required (use categoryName or categoryId).",
      400,
    );
  }
    ...
    runTool(userId, () =>
      productService.getSellerProducts(
        buildSellerReq(userId, {
          query: { search: args.search, page: args.page },
        }),
      ),
    ),
      ...
      parameters: {
        ...
        properties: {
          imageToken: {
            type: "string",
            description:
              "Optional. Only if image was uploaded outside this chat session.",
          },
          name: { type: "string" },
          price: { type: "number" },
          categoryName: {
